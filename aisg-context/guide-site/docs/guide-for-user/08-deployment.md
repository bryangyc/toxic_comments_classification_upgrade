...
=== "Linux/macOS/VSCode Server Terminal"
    ...
    cd src
    gunicorn toxic_comments_severity_fastapi.main:APP -b 0.0.0.0:8080 -w 2 -k uvicorn.workers.UvicornWorker -t 90
    ```
...
=== "Windows PowerShell"
    ...
    conda activate toxic-comments-severity
    cd src
    uvicorn toxic_comments_severity_fastapi.main:APP
    ```
...
`src/toxic_comments_severity_fastapi/config.py`. This class 
...
`src/toxic_comments_severity_fastapi/config.py`:
```python
...
class Settings(pydantic_settings.BaseSettings):

    API_NAME: str = "toxic_comments_severity_fastapi"
